...
//setup constants
config const r0 = toCodeLength(15.0); // radius of ring, in kpc
...
var pot = 7; //set galactic potential to imported csv box
...
config const Lbox: real = 2.0; //length of side of box in codeunits
...
proc pot_box(calcpar,pot) { //creates acceleration box in km/seconds squared
  ...
  var nfwbox = open("box-pot.csv",iomode.cw); //create test.csv and open
...
proc fwd_orbit (pos, vel, AM, SD, PS, pot, integrator, N, dt, calcpar,WritingChannel, SDWritingChannel,AMWritingChannel,PSWritingChannel) {
    if integrator == 0  { //if leapfrog
      ...
      for j in 1..Ne {
        halfstep(pos[j],vel[j],pot,dt,1.0, calcpar,pot_array,center_offset);
        AM[j] = angMom(pos[j],vel[j]);
...
proc main(){
  ...
  pot_box(Lbox, dt, calcpar,5, grid_size,pot_array, box_param);
  writeln("created box");
